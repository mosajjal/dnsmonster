...
ENGINE=SummingMergeTree
  PARTITION BY toYYYYMMDD(DnsDate)
  PRIMARY KEY  (timestamp, Server, IPVersion, IPPrefix)
  ORDER BY  (timestamp, Server, IPVersion, IPPrefix)
  SAMPLE BY IPPrefix
  TTL DnsDate + INTERVAL 30 DAY -- DNS_TTL_VARIABLE
  SETTINGS index_granularity = 8192
  AS SELECT DnsDate, timestamp, Server, IPVersion, IPPrefix, count(*) as c FROM DNS_LOG GROUP BY Server, DnsDate, timestamp, IPVersion, IPPrefix ;  
